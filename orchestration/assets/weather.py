"""Weather forcing extraction asset."""
...
def weather_forcing_raw(
    ...
    # Determine date range based on watermark and mode
    # Open-Meteo API only accepts dates up to yesterday, not today
    end_date = duckdb.get_reference_date() - timedelta(days=1)
    ...
    # Ensure start_date doesn't exceed end_date (can happen if watermark is
    # from a previous run with a later reference date)
    if start_date >= end_date:
        context.log.info(
            f"Data already up to date for reference period "
            f"(watermark={watermark}, end_date={end_date.date()})"
        )
        return MaterializeResult(
            metadata={
                "num_records": 0,
    ...
    context.log.info(
        f"Rate limiting: {config.calls_per_minute} calls/min, "
        f"{config.retry_attempts} retries with {config.retry_backoff}s backoff"
    )
    ...
    # Write to table (upsert based on key columns)
    new_records = duckdb.write_dataframe(
        df, TBL_WEATHER, schema=RAW_SCHEMA, key_columns=["longitude", "latitude", "datetime"]
    )
