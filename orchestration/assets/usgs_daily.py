"""USGS daily streamflow extraction asset."""
...
def usgs_streamflow_daily(
    context: AssetExecutionContext,
    config: StreamflowConfig,
    duckdb: DuckDBResource,
) -> MaterializeResult:
    """Extract daily streamflow values from USGS NWIS.

    Supports incremental loading:
    - First run: loads `days_back` days of history
    - Subsequent runs: loads from last date minus `incremental_days` overlap

    Uses (site_id, date) as the unique key to avoid duplicates.
    """
    from elt.extraction.usgs import fetch_usgs_daily
    ...
    watermark = get_high_watermark(duckdb, TBL_DAILY, "date")
    ...
    context.log.info(
        f"Fetching daily values for {len(site_ids)} sites "
        f"from {start_date.date()} to {end_date.date()}"
    )

    # Fetch in batches
    batch_size = 50  # Can use larger batches for daily data
    all_data = []

    for i in range(0, len(site_ids), batch_size):
        batch = site_ids[i : i + batch_size]
        context.log.info(f"Fetching daily batch {i // batch_size + 1}...")
        ...
        try:
            ...
            context.log.warning(f"Failed to fetch daily batch: {e}")
            continue
    ...
    new_records = upsert_timeseries(
        duckdb, df, TBL_DAILY, key_columns=["site_id", "date"]
    )
